...
def extract_info(filepath, base_folder, ctx):
    print(f"🔍 Scanning: {os.path.relpath(filepath, base_folder)}")
        ...
        if isinstance(item, ast.ClassDef):
            ...
            for child in item.body:
                if isinstance(child, ast.FunctionDef):
                    ...
                    write_function_note(child.name, child.args, rel_path, ctx, os.path.join(class_path, child.name))
            ...
            write_class_note(class_name, bases, methods, props, rel_path, ctx, class_path)
            ...
            write_function_note(func_name, item.args, rel_path, ctx, full_func_path)
            find_calls(item, full_func_path, ctx)

def write_class_note(name, bases, methods, props, rel_path, ctx, full_path):
...
def write_function_note(name, args_obj, rel_path, ctx, full_path):
    dest_path = os.path.join(ctx['VAULT_DIR'], full_path + ".md")
