...
def extract_info(filepath, base_folder):
    ...
    for item in node.body:
        if isinstance(item, ast.Import):
            for alias in item.names:
                import_data[rel_path].append(alias.name)
        elif isinstance(item, ast.ImportFrom):
            module = item.module if item.module else ""
            for alias in item.names:
                import_data[rel_path].append(f"{module}.{alias.name}")
        ...
        if isinstance(item, ast.ClassDef):
            ...
            for child in item.body:
                if isinstance(child, ast.FunctionDef):
                    ...
                    find_calls(child, rel_path_no_ext, class_name)
                elif isinstance(child, ast.Assign):
                    for target in child.targets:
                        if isinstance(target, ast.Name):
                            props.append(target.id)
            class_data[rel_path].append({
                "name": class_name,
                "bases": bases,
                "methods": methods,
                "properties": props
            ...
            class_index[class_name] = full_class_path
            module_index[rel_path]["classes"].append(full_class_path)

        elif isinstance(item, ast.FunctionDef):
            func_name = item.name
            function_data[rel_path].append({
                "name": func_name,
                "args": [arg.arg for arg in item.args.args]
            })
            full_func_path = os.path.join(rel_path_no_ext, func_name)
            function_index[func_name] = full_func_path
            module_index[rel_path]["functions"].append(full_func_path)
            find_calls(item, rel_path_no_ext, func_name)

def find_calls(node, current_path, current_name):
    for child in ast.walk(node):
        if isinstance(child, ast.Call):
            if isinstance(child.func, ast.Name):
                called = child.func.id
                if called in function_index:
                    uses_map[os.path.join(current_path, current_name)].add(function_index[called])
            elif isinstance(child.func, ast.Attribute):
                attr = child.func.attr
                if attr in function_index:
                    uses_map[os.path.join(current_path, current_name)].add(function_index[attr])
...
def generate_notes(base_folder):
    for rel_path, classes in class_data.items():
        for cls in classes:
            ...
            content += [f"- {resolve_link(class_index[b])}\n" for b in cls['bases'] if b in class_index] or ["- None\n"]
            content.append("\n## Methods:\n")
            content += [f"- `{m}` #method\n" for m in cls['methods']] or ["- None\n"]
            content.append("\n## Properties:\n")
            content += [f"- `{p}` #property\n" for p in cls['properties']] or ["- None\n"]
            if base_path in uses_map:
                content.append("\n## Uses:\n")
                content += [f"- {resolve_link(target)}\n" for target in uses_map[base_path]]
            _write_note(dest_path, content)

    for rel_path, funcs in function_data.items():
        for func in funcs:
    ...
    for rel_path, items in module_index.items():
    ...
    for rel_path, imports in import_data.items():
...
def scan_folder(folder):
    for root, _, files in os.walk(folder):
        for file in files:
            if file.endswith(".py"):
                extract_info(os.path.join(root, file), folder)

scan_folder(PROJECT_PATH)
generate_notes(PROJECT_PATH)
